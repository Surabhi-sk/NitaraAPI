...
public class AddDeworming extends GenericBase{
	...
	@Test(groups= {"Smoke"})
	public void addDeworming() throws Exception  {

		String abstractname = prop.getProperty("AddDeworming");
		...
		JSONObject dataObject = data.readData("AddDeworming",filepath);
		...
		Assert.assertEquals("Deworming Data Added Successfully.", message);


		//Verify the added details
		ViewDewormingData details = new ViewDewormingData();
		String response = details.viewDewormingData(dataObject.getString("CattleIds"), token);
